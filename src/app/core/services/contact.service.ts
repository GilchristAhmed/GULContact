...
//rechercher un contact
  searchContacts(page:number=1,query: string, token: string): Observable<{ data: Contact[] }> {
    return this.http.get<{ data: Contact[] }>(`${this.apiUrl}?page=${page}search=${query}`, this.getHeader(token));
  }
//Selection un contact en fontion de son ID
  ...
  //récuperer les données pour la page suivante
